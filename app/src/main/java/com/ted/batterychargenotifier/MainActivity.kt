...
class MainActivity : AppCompatActivity() {
    companion object {
        const val PREFS_NAME = "BatteryPrefs"
        const val CHANNEL_ID = "ChargeNotify"
        private const val REQ_POST_NOTIFICATIONS = 1001
        private const val KEY_STARTUP_NOTIFY_ENABLED = "startup_notify_enabled"
    ...
    private fun createNotification(): Boolean    {
        val settings = getSharedPreferences(PREFS_NAME, MODE_PRIVATE)
        if (!settings.getBoolean(KEY_STARTUP_NOTIFY_ENABLED, true)) {
            return true
        }
